...
from .cog_sci_learning_model_base import (MultiArmedBanditModels, load_matlab_data, load_data, get_actions_rewards)

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

class ContextualRascorlaWagnerChoiceKernelPlusInitModel(MultiArmedBanditModels):
    ...
    def crwck_plus_init_exp(self, m=4):
        ...
        alpha_bounds = (0.01,1)
        theta_bounds = (0.01,5)
        alpha_init = np.linspace(0.01,1,m)
        theta_init = np.linspace(0.01,5,m)
        _bounds = (alpha_bounds,alpha_bounds, theta_bounds,theta_bounds)
        counter = 0

        crwck_easy_df = pd.read_csv('data/data_frames/pre_sessions/df_crwck_plus_pre_init.csv')
        crwck_hard_df = pd.read_csv('data/data_frames/pre_sessions/df_crwck_plus_pre_init.csv')
        ...
        output_path = os.path.join(PROJECT_ROOT, 'data', 'data_frames', 'df_crwck_plus_init.csv')
