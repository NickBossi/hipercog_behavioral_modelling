...
from .cog_sci_learning_model_base import (MultiArmedBanditModels, load_data, load_matlab_data, get_actions_rewards)

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

class ContextualRascorlaWagnerChoiceKernelPlusSharedAlphaModel(MultiArmedBanditModels):
    ...
    def simulate(self, alpha, beta_rw, beta_ck, N=100, Q_init=[0, 0], noise=True):
        """
    ...
    def perform_sensitivity_analysis(
        ...
        for alpha, beta_rw_true, beta_ck_true in param_grid:
            ...
            # Store results
            results['N'].append(N)
            results['alpha (true)'].append(alpha)
    ...
    def probs_given_data(self, resp_codes, stim_codes, actions, rewards, params, Q_init = [0,0]):
        ...
        for t in range(N):
            if stim_codes[t] == 0:
                ...
                Q[0][actions[t]] = Q[0][actions[t]] + alpha*delta
                ...
                Q[1][actions[t]] = Q[1][actions[t]] + alpha*delta
    ...
    def neg_log_likelihood(self, parameters, stim_codes, actions, rewards, Q_init = [0,0]):
        alpha, beta_rw, beta_ck = parameters
        Q = [Q_init,Q_init]
        N = len(actions)
        CK = [[0,0],[0,0]]
        log_likelihood = 0
        ...
        for _alpha in alpha_iterable:
                for _theta_rw in theta_range:
                    for _theta_ck in theta_range:
                        yield _alpha, _theta_rw, _theta_ck
    ...
    def optimize_scikit_model_over_init_parameters(
        ...
        # run experiments
        for _alpha, _theta_rw, _theta_ck in gen_experiments:              
            result, res_nll, param_fits = self.optimize_scikit(
                loss_function=loss_function,
                init_guess=[_alpha, _theta_rw, _theta_ck],
            ...
            if result.fun < negLL:
                negLL = result.fun
                params_opt = result.x
                optimal_init_params = (_alpha, _theta_rw, _theta_ck)
      
        # compute BIC

        BIC = self.compute_BIC(negLL, len(actions), 3)
        return {'negLL': negLL, 'param_opt': params_opt, 'BIC': BIC, 'optimal_init_params': optimal_init_params, 'n': len(actions)}
     
    def crwcksa_plus_exp(self, m=5):
        ...
        _bounds = (alpha_bounds, theta_bounds,theta_bounds)
        counter = 0
        ...
        for k,v in EDS_Easy.items():
            ...
            new_res = {
                ...
                'beta_rw_opt': res['param_opt'][1],
                'beta_ck_opt': res['param_opt'][2],
                'BIC': res['BIC'],
                'n': res['n'],
        ...
        output_path = os.path.join(PROJECT_ROOT, 'data', 'data_frames', 'df_crwcksa_plus.csv')
